...
class VirtualSimulator:
    ...
    def step(self, dt: Optional[float] = None) -> Dict[str, float]:
        ...
        target_airspeed = self.parameters['throttle_to_speed'] * self.controls['throttle']
        airspeed_change = (target_airspeed - current_state['airspeed']) * self.parameters['throttle_response'] * dt
        new_airspeed = current_state['airspeed'] + airspeed_change
        
        # Calculate pitch rate based on elevator input and current pitch
        elevator_pitch_rate = self.parameters['elevator_to_pitch_rate'] * self.controls['elevator']
        ...
        aileron_roll_rate = self.parameters['aileron_to_roll_rate'] * self.controls['aileron']
        ...
        rudder_heading_rate = self.parameters['rudder_to_heading_rate'] * self.controls['rudder']
        ...
        # Simple flat earth approximation for educational purposes
        lat_change = distance * np.cos(heading_rad) / 60.0  # 1 minute of lat = 1 nm
        lon_change = distance * np.sin(heading_rad) / (60.0 * np.cos(np.radians(current_state['latitude'])))
